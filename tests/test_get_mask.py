...
from bots.tools.prepare.generate_wordcloud_mask import generate_wordcloud_mask
...
class TestGetMask(unittest.TestCase):
  ...
  def test1(self):
    ...
    input.state.wordcloud_text = wordcloud_text
    input.state.wordcloud_counts = wordcloud_counts
    generate_wordcloud_mask(input)
    self.assertEqual(input.state.wordcloud_width, 1024)
    self.assertEqual(input.state.wordcloud_height, 1024)
