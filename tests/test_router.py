...
class TestRouter(unittest.TestCase):
  ...
  def test_digest_casts(self):
    ...
    action = route(request)
    action.print()
    self.assertIsInstance(action, action.digest_casts.DigestCasts)
    
  def test_favorite_users(self):
    request = "Who are @vitalik.eth's favorite users?"
    action = route(request)
    action.print()
    self.assertIsInstance(action, action.favorite_users.FavoriteUsers)
  
  def test_more_like_this(self):
    request = "More Like This: #Bitcoin is sweet!"
    action = route(request)
    action.print()
    self.assertIsInstance(action, action.more_like_this.MoreLikeThis)
    
  def test_most_active_users(self):
    request = "Who is most active in channnel /data?"
    action = route(request)
    action.print()
    self.assertIsInstance(action, action.most_active_users.MostActiveUsers)
    
  def test_pick_cast(self):
    request = "Pick the funniest cast in channnel /data?"
    action = route(request)
    action.print()
    self.assertIsInstance(action, action.pick_cast.PickCast)
    
  def test_prefs_cloud(self):
    request = "Make a wordcloud for user @vitalik.eth"
    action = route(request)
    action.print()
    self.assertIsInstance(action, action.word_cloud.WordCloud)

  def test_psycho(self):
    request = "Psycho analyze @v"
    action = route(request)
    action.print()
    self.assertIsInstance(action, action.psycho.Psycho)
    
  def test_roast(self):
    request = "Roast @v"
    action = route(request)
    action.print()
    self.assertIsInstance(action, action.roast.Roast)

  def test_perplexity(self):
    request = "Ask perplexity to live or not to live"
    action = route(request)
    action.print()
    self.assertIsInstance(action, action.perplexity.Perplexity)

  def test_news(self):
    request = "Data Science news"
    action = route(request)
    action.print()
    self.assertIsInstance(action, action.news.News)