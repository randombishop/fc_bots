...
def parse_digest_casts_params(input):
  state = input['state']
  llm = input['llm']
  ...
  _, user_name = read_user(params, fid_origin=state.fid_origin, default_to_origin=False)
  parsed['user_name'] = user_name
  state.action_params = parsed
  return {"action_params": parsed}


ParseDigestCastsParams = Tool(
  name="parse_digest_casts_params",
  func=parse_digest_casts_params,
